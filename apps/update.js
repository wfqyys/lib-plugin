...
const resPath = `${_path}/plugins/lib-plugin/resources/`
...
export class Update extends plugin {
  ...
  async runUpdate (isForce) {
    const _path = './plugins/lib-plugin/'
    ...
    this.oldCommitId = await this.getcommitId('lib-plugin')
    uping = true
    let ret = await this.execSync(command)
    uping = false

    if (ret.error) {
      logger.mark(`${this.e.logFnc} 更新失败：lib-plugin`)
      this.gitErr(ret.error, ret.stdout)
      return false
    }

    /** 获取插件提交的最新时间 */
    let time = await this.getTime('lib-plugin')

    if (/(Already up[ -]to[ -]date|已经是最新的)/.test(ret.stdout)) {
      await this.reply(`lib-plugin已经是最新版本\n最后更新时间：${time}`)
    } else {
      await this.reply(`lib-plugin\n最后更新时间：${time}`)
      this.isUp = true
      /** 获取星铁组件的更新日志 */
      let log = await this.getLog('lib-plugin')
  ...
  async getLog (plugin = '') {
    ...
    let end = ''
      ...
      '更多详细信息，请前往gitee查看\nhttps://github.com/wfqyys/lib-plugin'

    log = await this.makeForwardMsg(`lib-plugin更新日志，共${line}条`, log, end)
