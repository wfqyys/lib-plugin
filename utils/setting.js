...
class Setting {
  constructor () {
    /** 默认设置 */
    this.defPath = `${_path}/plugins/lib-plugin/defSet/`
    this.defSet = {}

    /** 用户设置 */
    this.configPath = `${_path}/plugins/lib-plugin/config/`
    this.config = {}

    this.dataPath = `${_path}/plugins/lib-plugin/data/`
    this.data = {}
  ...
  getFilePath (app, type) {
    if (type === 'defSet') return `${this.defPath}${app}.yaml`
      ...
      try {
        ...
        logger.error(`lib-plugin缺失默认文件[${app}]${error}`)
      }
  ...
  watch (file, app, type = 'defSet') {
    ...
    watcher.on('change', path => {
      ...
      logger.mark(`[lib-plugin][修改配置文件][${type}][${app}]`)
      if (this[`change_${app}`]) {
        this[`change_${app}`]()
      }
    })
