...
async function setUserData(qq, data) {
  const dir = 'resources/LibCache/user'
...
export async function createServer() {
  ...
  await server.register(fstatic, {
    root: path.join(__dirname, 'plugins/lib-plugin/server/static/')
  })
  await server.register(fastifyCookie)
  await server.get('/page/*', (request, reply) => {
    const stream = fs.createReadStream('plugins/lib-plugin/server/static/index.html')
  ...
  server.post('/login', async (request, reply) => {
    ...
    if (body.qq && body.passwd) {
      ...
      if (body.qq == Bot.uin && await redis.get('lib:ADMIN_PASSWD') == body.passwd) {
  ...
  server.post('/page', async (request, reply) => {
    const body = request.body || {}
    if (body.code) {
      const dir = 'resources/libCache/page'
  ...
  server.post('/help', async (request, reply) => {
    const body = request.body || {}
    if (body.use) {
      const dir = 'plugins/lib-plugin/resources'
