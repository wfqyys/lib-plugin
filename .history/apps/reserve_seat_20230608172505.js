...
// 获取个人信息与预约信息字典
function getData() {
  const users = JSON.parse(fs.readFileSync('./Lib/users_dict.dll', 'utf-8'));
  const dataDict = JSON.parse(fs.readFileSync('./Lib/data_dict.dll', 'utf-8'));
...
async function main() {
  ...
  if (status === 'success') {
    ...
    printResult(users, status, null, null, null, null, res.message);
  }
}
